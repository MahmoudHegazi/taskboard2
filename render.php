...
if ($teamidResult->num_rows > 0) {
  ...
  $sql = "SELECT  user.id, user.name, day.id AS 'day_id', day.title AS 'day_title', (SELECT team.title FROM team WHERE team.id = $selectedteamid) AS 'groupname', (SELECT team.classname FROM team WHERE team.id = $selectedteamid) AS 'teamclass' FROM user INNER JOIN day WHERE day.week_id = 1 AND user.team_id = $selectedteamid";
